...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    switch (name) {
      case "get_stories": {
        const result = await client.getStories(args?.status as string | undefined, args?.assignee as string | undefined);
        return {
          content: [
            {
              type: "text",
      ...
      case "add_story": {
        const result = await client.addStory(args?.title as string, args?.description as string | undefined);
        return {
          content: [
            {
              type: "text",
      ...
      case "update_story_status": {
        const result = await client.updateStoryStatus(args?.id as string, args?.status as "TODO" | "IN_PROGRESS" | "DONE");
        return {
          content: [
            {
              type: "text",
