...
async function runTests() {
	...
	try {
		...
		// Test 3: Add story
		console.log("\n3. Testing addStory()...");
		const newStory = await client.addStory(
			"Test story from MCP client",
		...
		console.log(
			`   ✅ Claimed: ${claimed.id} -> ${claimed.status} (assignee: ${claimed.assignee})`,
		);
		...
		console.log(
			`   ✅ Unclaimed: ${unclaimed.id} -> ${unclaimed.status} (assignee: ${unclaimed.assignee})`,
		);
