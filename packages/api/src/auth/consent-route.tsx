 ...
 * non-admins, and renders the branded consent page via the
 * `@elysiajs/html` plugin (which auto-sets `Content-Type: text/html` on the
 * Response when a JSX string is returned).
 *
 * The pure JSX renderer lives in `consent-page.tsx`; this file owns only
 * the routing + dependency-resolution glue.
 *
 * Behaviour:
...
import { type OAuthClientRecord, renderConsentPage } from '@packrat/api/auth/consent-page';
...
export const consentRoute = new Elysia()
  .use(html())
  .get('/oauth/consent', async ({ request, query, set }) => {
    ...
    if (!clientRow) {
      set.status = 404;
      return <span safe>{`Unknown OAuth client: ${clientId}`}</span>;
    }
    ...
    // Security headers — the html() plugin sets Content-Type; the rest are
    // OAuth-flow defaults that mirror what the legacy bypass-handler emitted.
