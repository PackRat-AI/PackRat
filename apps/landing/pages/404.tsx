import { Backpack } from 'lucide-react';
import Head from 'next/head';

export default function Custom404() {
  return (
    <>
      <Head>
        <title>Page not found | PackRat</title>
        <meta
          name="description"
          content="The page you're looking for doesn't exist on PackRat. Head back home or explore what PackRat can do for your next outdoor adventure."
        />
        <meta name="robots" content="noindex,nofollow" />
      </Head>
      <main
        style={{
          minHeight: '100vh',
          display: 'flex',
          alignItems: 'center',
          justifyContent: 'center',
          background: '#0f172a',
          fontFamily: 'system-ui, sans-serif',
        }}
      >
        <div style={{ textAlign: 'center', padding: '0 1rem' }}>
          <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '1.5rem' }}>
            <div
              style={{
                background: 'rgba(15,118,110,0.15)',
                borderRadius: '9999px',
                padding: '1.5rem',
                display: 'inline-flex',
              }}
            >
              <Backpack size={48} color="#14b8a6" aria-hidden="true" />
            </div>
          </div>
          <h1
            style={{
              fontSize: '5rem',
              fontWeight: 800,
              color: '#f8fafc',
              margin: '0 0 0.5rem',
              lineHeight: 1,
            }}
          >
            404
          </h1>
          <p
            style={{ fontSize: '1.25rem', fontWeight: 600, color: '#f8fafc', margin: '0 0 0.5rem' }}
          >
            Trail not found
          </p>
          <p style={{ color: '#94a3b8', maxWidth: '22rem', margin: '0 auto 2rem' }}>
            Looks like you&apos;ve wandered off the map. This page doesn&apos;t exist or has moved.
          </p>
          <div
            style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem', justifyContent: 'center' }}
          >
            <a
              href="/"
              style={{
                background: '#0f766e',
                color: '#fff',
                padding: '0.625rem 1.5rem',
                borderRadius: '0.5rem',
                fontWeight: 600,
                textDecoration: 'none',
                fontSize: '1rem',
              }}
            >
              Back to home
            </a>
            <a
              href="/#faq"
              style={{
                border: '1px solid #334155',
                color: '#f8fafc',
                padding: '0.625rem 1.5rem',
                borderRadius: '0.5rem',
                fontWeight: 600,
                textDecoration: 'none',
                fontSize: '1rem',
              }}
            >
              Browse FAQ
            </a>
          </div>
        </div>
      </main>
    </>
  );
}
