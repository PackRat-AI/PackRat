  ...
  it('accepts a token in the JSON body when the auth header is absent', async () => {
    await expect(
      verifyDeployedAuth(
        ...
        async () => response({ ok: true, body: { user: { id: 'user-1' }, token: 'body-token' } }),
      ),
  ...
  it('falls back to status when the error body is not JSON', async () => {
    await expect(
      verifyDeployedAuth(
        ...
        async () => response({ ok: false, status: 503, jsonThrows: true }),
      ),
  ...
  it('fails when the deployed response lacks a user or session token', async () => {
    await expect(
      verifyDeployedAuth(
        ...
        async () => response({ ok: true, body: { user: { id: 'user-1' } } }),
      ),
