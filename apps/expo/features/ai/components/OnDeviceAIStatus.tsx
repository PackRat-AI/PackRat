...
export function OnDeviceAIStatus({ onConfigurePress }: OnDeviceAIStatusProps) {
  ...
  const { capabilities, isLoading, error, isOnDeviceAvailable, recommendedProvider, refresh } = useOnDeviceAI();
  ...
  return (
    <View className=\"flex-row items-center gap-2 p-3 bg-success/10 rounded-lg\">
      <Icon 
        name={getProviderIcon(recommendedProvider!)} 
        size={16} 
  ...
  return (
    <View className=\"space-y-2 p-3 bg-card rounded-lg\">
      ...
      <View className=\"space-y-1\">
        ...
        <CapabilityRow 
          label=\"MLC Engine\" 
          available={capabilities.supportsMLC}
        />
      </View>
