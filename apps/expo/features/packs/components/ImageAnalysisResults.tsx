  ...
  if (detectedItems.length === 0) {
    return (
      <View className="flex-1 items-center justify-center p-6">
        ...
        <Text className="mt-4 text-center text-foreground">No outdoor gear detected</Text>
        <Text className="mt-2 text-center text-sm text-muted-foreground">
...
function DetectedItemCard({ item, onItemSelect, onSimilarItemSelect }: DetectedItemCardProps) {
  ...
  return (
    <View className="mb-4 rounded-lg bg-card">
      {/* Main detected item */}
      <Pressable onPress={onItemSelect} className="p-4 active:opacity-70">
            ...
            <View className="mt-2 flex-row flex-wrap gap-2">
              ...
              {item.brand && (
                <View className="rounded bg-secondary px-2 py-1">
                  <Text className="text-xs text-secondary-foreground">{item.brand}</Text>
                </View>
              )}
              {item.color && (
                <View className="rounded bg-accent px-2 py-1">
                  <Text className="text-xs text-accent-foreground">{item.color}</Text>
                </View>
              )}
            </View>
          </View>
        ...
        <View className="border-t border-border p-4 pt-3">
          ...
          <FlatList
            ...
            renderItem={({ item: result }) => (
              <SimilarItemCard item={result} onPress={() => onSimilarItemSelect(result)} />
            )}
...
function SimilarItemCard({ item, onPress }: SimilarItemCardProps) {
  ...
  return (
    <Pressable onPress={onPress} className="w-48 rounded-lg bg-muted p-3 active:opacity-70">
      {item.imageUrl && (
        <Image
      ...
      <View className="mt-2 flex-row items-center justify-between">
        {item.price && <Text className="text-sm font-semibold text-primary">${item.price}</Text>}
        <View className="flex-row items-center">
