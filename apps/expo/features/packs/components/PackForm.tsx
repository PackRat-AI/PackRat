...
import {
  ...
  FormItem,
  FormSection,
} from "expo-app/components/nativewindui/Form";
...
export const PackForm = ({ pack }: { pack?: Pack }) => {
  ...
  const templateAtom = isCreatingFromTemplate
    ? packTemplatesStore[templateId]
    : null;
  const template = templateAtom ? templateAtom.get() : null;

  const templateItems = isCreatingFromTemplate
    ? getTemplateItems(params.templateId as string)
    : [];
  ...
  return (
    ...
    >
      {isCreatingFromTemplate && (
        <Stack.Screen options={{ title: "Enter New Pack Details" }} />
      )}
      {isCreatingFromTemplate && template && (
