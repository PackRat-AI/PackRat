...
export default function LocationDetailScreen() {
  ...
  const setAsActive = () => {
    if (location.isActive) {
      Alert.alert(
        "Already Active",
        `${location.name} is already set as your active location.`,
        [{ text: "OK" }]
      );
      return;
    }
    ...
    Alert.alert(
      "Location Set",
      `${location.name} has been set as your active location.`,
      [{ text: "OK" }]
    );
  };
                ...
                <View className="mt-4 rounded-xl bg-white/10 p-4">
                  <Text className="mb-2 font-medium text-white">
                    {location.dailyForecast
                      ? `${location.dailyForecast.length}-DAY`
                      : "DAILY"}{" "}
                    FORECAST
                  </Text>
                  {location.dailyForecast ? (
                  ...
                  <View className="flex-row flex-wrap">
                    ...
                    <View className="w-1/2 p-2">
                      ...
                      <Text className="text-xl text-white">
                        ...
                        {location.details?.uvIndex &&
                        location.details.uvIndex > 5
                          ? "(High)"
                          : ""}
                      </Text>
                    </View>
