...
describe('getRelativeTime', () => {
  ...
  it('returns "Just now" for very recent timestamps (< 1 minute ago)', () => {
    ...
    const result = getRelativeTime({ dateValue: '2024-01-01T12:00:00Z' });
  ...
  it('returns "Just now" for future timestamps (diff < 0)', () => {
    ...
    const result = getRelativeTime({ dateValue: '2024-01-01T13:00:00Z' });
    expect(result).toBe('Just now');
  });
  ...
  it('returns "Just now" for invalid Date instances without translation', () => {
    expect(getRelativeTime({ dateValue: new Date('not-a-date') })).toBe('Just now');
  });
