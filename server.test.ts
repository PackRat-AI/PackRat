...
interface TestData {
	...
	comments: Record<string, TestComment[]>;
}
...
let testData: TestData = { stories: [], agents: {}, comments: {} };
...
function startServer() {
	server = Bun.serve({
		...
		async fetch(req) {
			...
			const findStory = (stories: TestStory[], id: string) => stories.findIndex((s) => s.id === id);
			...
			if (method === "GET" && path === "/stories") {
				return json({
					stories: testData.stories.map((s) => ({ ...s, status: normalizeStatus(s.status) })),
				});
			}
			...
			if (method === "GET" && path.startsWith("/stories/")) {
				...
				const story = {
					...testData.stories[idx],
					status: normalizeStatus(testData.stories[idx].status),
				};
			...
			if (method === "GET" && path === "/agents") {
				const agents = Object.entries(testData.agents || {}).map(([id, agent]) => ({
					id,
					...agent,
			...
			if (method === "GET" && path === "/board") {
				const stories = testData.stories.map((s) => ({ ...s, status: normalizeStatus(s.status) }));
				const agents = Object.entries(testData.agents || {}).map(([id, agent]) => ({
					id,
					...agent,
					lastSeen: agent.last_seen,
...
describe("Board", () => {
	...
	test("GET /board returns full state with normalized statuses", async () => {
		...
		expect(res.data.agents).toBeDefined();
		const story = res.data.stories.find((s: { title: string }) => s.title === "Board Story");
