...
let envFileContent = fs.readFileSync(envSource.path, 'utf8');

// A `devenv` environment allocates its own API port (8788+ so parallel agents
// do not collide), so the API URL cannot be a static value in the shared
// .env.local. When DEVENV_API_URL is set, it overrides the source's value for
// this generation only — the shared file is never written to.
const devenvApiUrl = process.env.DEVENV_API_URL;
if (devenvApiUrl) {
  const overrides: Record<string, string> = {
    PUBLIC_API_URL: devenvApiUrl,
    EXPO_PUBLIC_API_URL: devenvApiUrl,
  };
  for (const [key, value] of Object.entries(overrides)) {
    const line = `${key}=${value}`;
    const pattern = new RegExp(`^${key}=.*$`, 'm');
    envFileContent = pattern.test(envFileContent)
      ? envFileContent.replace(pattern, line)
      : `${envFileContent}\n${line}`;
  }
  console.log(`   Pointing clients at the devenv API: ${devenvApiUrl}`);
}
 ...
 */
const nextJsApps = ['admin', 'landing', 'guides', 'trails', 'web'] as const;
